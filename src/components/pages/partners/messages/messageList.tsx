...
export function PartnerMessagesList(mainprop:{pbi:partnerBasicinfo,actiony:(thread:msgThread, action:number)=>void, backy:()=>void}){
    ...
    function handleError(task:resHandler){
        ...
        setError(true)
        if(task.isLoggedOut()){
            navigate(`/partnerLogin?rdr=${location.pathname.substring(1)}`)
        }else{
        ...
        <div className="ctr" style={{
            ...
            <PickMsgReceiver closy={()=>{
                ...
                makeRequest.post('createMsgThread',{
                    from: mainprop.pbi.getFirstName(),
                    from_uid: mainprop.pbi.getPartnerID(),
                    to: 'Admin',
                    to_uid: masterID,
                    last_msg:'New Message',
                    subject: subject,
                    from_mail: mainprop.pbi.getEmail(),
                    to_mail:masterEmail
                },(task)=>{
                    setLoad(false)
                    if(task.isSuccessful()){
